...
export const Editorial: React.FC = () => {
  ...
  useEffect(() => {
    updateWidth();
    // Add resize listener logic if needed, usually managed by state update
  ...
  return (
    <section id="editorial" ref={containerRef} className="flex flex-col items-center w-full px-4 md:px-10 lg:px-20 pb-20 pt-10 bg-chalk">
      <div className="w-full max-w-[1440px]">
        ...
        <div className="flex flex-col gap-2 pt-20 pb-12 md:pt-32 md:pb-24">
          <motion.div
            ...
            variants={fadeUp}
          >
            <h1 className="text-5xl md:text-7xl lg:text-9xl font-serif tracking-tighter leading-none text-text-main">
              ARCÈL 01 — <br />
              <span className="text-primary italic font-light opacity-90">STILLNESS</span>
            </h1>
            <div className="flex items-center gap-4 mt-6">
              <span className="h-[1px] w-12 bg-primary"></span>
              <p className="text-text-muted text-sm md:text-base font-medium tracking-widest uppercase">FALL 2026 COLLECTION</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="w-full mb-24 md:mb-40">
          <motion.div
            className="relative w-full aspect-[3/4] md:aspect-[21/9] overflow-hidden rounded-sm group"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 p-6 md:p-10">
              <p className="text-white/90 text-xs font-medium tracking-wider uppercase backdrop-blur-md bg-black/20 px-3 py-1 rounded-sm inline-block">
                Campaign 01
              </p>
            </div>
          </motion.div>
        ...
        <div className="flex justify-center mb-24 md:mb-40 px-4">
          <motion.div
            ...
            variants={fadeUp}
          >
            <h2 className="text-3xl md:text-5xl font-serif text-text-main leading-tight mb-6">
              "Weightless wool. <br /> <span className="text-primary-light font-normal italic">Structured silence."</span>
            </h2>
        ...
        <div className="flex flex-col gap-12 md:gap-32 w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-end">
            <motion.div
              ...
              variants={fadeUp}
            >
              <div className="w-full aspect-[4/5] md:aspect-[16/10] bg-background-subtle rounded-sm overflow-hidden">
                <motion.img
                  style={{ y: yParallax }}
                  loading="lazy"
                  ...
                  className="h-[110%] w-full object-cover -mt-[5%]"
                />
              </div>
              <div className="absolute -bottom-6 -right-2 md:-right-8 bg-background-light border border-text-main/5 p-6 z-10 max-w-[240px] shadow-sm">
                <p className="text-primary text-xs font-bold uppercase tracking-wider mb-2">Look 01</p>
                <p className="text-base font-serif text-text-main">Washed Linen Coat</p>
              </div>
            </motion.div>

            <div className="hidden md:block md:col-span-4 h-full pl-8 flex flex-col justify-end pb-12">
              <p className="text-xs text-text-muted uppercase tracking-widest rotate-90 origin-bottom-left translate-x-4 whitespace-nowrap">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 w-full pt-10">
            {/* Spacer/Text column */}
            <div className="md:col-span-4 flex flex-col justify-center order-2 md:order-1 px-4 md:px-0">
              <div className="h-[1px] w-20 bg-text-main/20 mb-8"></div>
              <p className="text-xl md:text-2xl font-serif text-text-main leading-relaxed mb-6">
                Raw edges meet precise tailoring. A study in contradiction.
              </p>
              <span className="text-xs text-text-muted font-mono mb-12 block">FABRIC: JAPANESE COTTON</span>

              {/* Desktop Navigation */}
              <div className="hidden md:flex gap-4">
                <button
                  onClick={() => slide('left')}
                  className="p-4 rounded-full border border-text-main/10 hover:border-primary hover:text-primary transition-colors duration-300"
                >
            ...
            <div className="md:col-span-8 order-1 md:order-2 relative" ref={carouselRef}>
              ...
              >
                ...
                >
                  {carouselItems.map((item, index) => (
                    <motion.div
                      key={item.id}
                      className="min-w-[85vw] md:min-w-[45%] relative aspect-[3/4] bg-background-subtle rounded-sm overflow-hidden shadow-sm group"
                      initial="hidden"
                      whileInView="visible"
                      viewport={{ once: true }}
                    ...
                    >
                      ...
                      <div className="absolute bottom-6 left-6 text-white translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                        <p className="text-xs font-bold uppercase tracking-wider opacity-80 mb-1">Look {item.id}</p>
              ...
              <div className="flex md:hidden justify-end gap-2 mt-4">
                <button
                  onClick={() => slide('left')}
                  className="p-2 rounded-full bg-white shadow-sm text-text-main active:scale-95 transition-transform"
                >
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <div className="w-full md:w-3/4 aspect-video bg-background-subtle rounded-sm overflow-hidden relative group cursor-crosshair">
              <img
                loading="lazy"
              ...
              >
                <span className="text-white font-medium tracking-widest uppercase text-xs border border-white/50 px-6 py-3 rounded-full bg-white/10 backdrop-blur-md">View Texture</span>
              </motion.div>
            </div>
            <p className="mt-6 text-xs text-text-muted font-mono tracking-widest">FIG 04. — TEXTURE DETAIL</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 w-full items-start">
            ...
            <motion.div
              ...
              variants={fadeUp}
            >
              <div className="w-full aspect-[4/3] bg-background-subtle rounded-sm overflow-hidden">
                <img
                  loading="lazy"
                ...
                />
                <div className="absolute bottom-6 left-6 text-white bg-black/20 px-4 py-2 rounded backdrop-blur-md border border-white/10">
                  <p className="text-xs font-bold uppercase tracking-wider mb-1">Look 04</p>
                  <p className="text-base font-serif">Evening Pleats</p>
                </div>
              </div>
            </motion.div>

            {/* Right Column: Stacked Images */}
            <div className="md:col-span-4 flex flex-col gap-8 md:mt-20">
              <motion.div
                className="relative w-full aspect-[3/4] bg-background-subtle rounded-sm overflow-hidden"
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-0 md:gap-12 items-center mb-20">
            <div className="md:col-span-5 md:col-start-2 z-10 p-6 md:p-0">
              <h3 className="text-4xl md:text-6xl font-serif font-light mb-6 text-text-main">Functional<br /><span className="italic">Minimalism.</span></h3>
              <p className="text-text-muted mb-10 max-w-sm leading-relaxed">
                Designed for movement. Constructed for longevity. Every seam serves a purpose, every fold a decision.
              </p>
              <div className="flex items-center gap-3 text-primary cursor-pointer hover:underline underline-offset-4 group transition-all">
            ...
            >
              <div className="w-full aspect-[3/4] md:aspect-[4/5] bg-background-subtle rounded-sm overflow-hidden shadow-xl">
                <img
                  loading="lazy"
              ...
              <div className="flex justify-between items-start mt-6 px-2">
                <div>
                  <p className="text-primary text-xs font-bold uppercase tracking-wider mb-1">Look 05</p>
                  <p className="text-base font-medium text-text-main">Canvas Work Jacket</p>
                </div>
                <p className="text-xs text-text-muted">05 / 12</p>
              </div>
            </motion.div>
          </div>
